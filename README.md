...
2. Set `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` (`pk_live_…`) and `STRIPE_SECRET_KEY` (`sk_live_…` or restricted `rk_live_…` with **Checkout Sessions** read/write).
3. Set `NEXT_PUBLIC_APP_URL` to your public site (e.g. `https://www.musiccityscavengerhunt.com`).
4. Run `npm run stripe:verify` to confirm keys load.
5. Stripe Dashboard → **Webhooks** → add endpoint `https://www.musiccityscavengerhunt.com/api/webhooks/stripe`, event **`checkout.session.completed`**, copy signing secret to `STRIPE_WEBHOOK_SECRET` (also add in Vercel/host env vars).

After payment, Stripe redirects to `/booking/success` with `session_id`; the app fulfills the booking there even before the webhook is configured. The webhook is still recommended for reliability.
