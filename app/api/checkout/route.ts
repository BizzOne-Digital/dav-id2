...
export async function POST(request: Request) {
  try {
    ...
    const successUrl =
      ...
      `${origin}/booking/success?bookingId=${booking._id}&session_id={CHECKOUT_SESSION_ID}`;
    ...
    const stripeMessage =
      err && typeof err === "object" && "type" in err && (err as { message?: string }).message
        ? String((err as { message?: string }).message)
        : null;
    console.error("[checkout]", err);
    const clientMessage =
      stripeMessage?.includes("Invalid API Key") || stripeMessage?.includes("api_key")
        ? "Payment provider configuration error. Check Stripe secret key permissions."
        : "Checkout failed";
